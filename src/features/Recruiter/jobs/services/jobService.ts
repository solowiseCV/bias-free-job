...
async getJobPostings(
  ...
  const companyProfile = await prisma.companyProfile.findFirst({
    where: { userId },
    include: { jobPostings: true }, // Include job postings to confirm ownership
  });
  if (!companyProfile) return { jobs: [], total: 0, page: page || 1, limit: limit ? parseInt(limit as any) : 10 };

  const whereClause: Prisma.JobPostingWhereInput = {
    companyProfileId: companyProfile.id, // Explicitly tie to the user's company
  };
  ...
  async updateJobPostingToDraft(userId: string, jobId: string, data: Partial<UpdateJobPostingDTO>) {
    const companyProfile = await prisma.companyProfile.findFirst({
      where: { userId },
    });
